...
static DMWeakReferencesArray *manageDelegates;// 管理类代理

@implementation UIApplication (DMManager)
...
- (void)registerCommonManager:(NSMutableArray<Class> *)list {
    [list addObject:[DMNetworkManager class]]; // 注册网络管理类
}

- (void)addManagersDelegate:(id <UIApplicationManagerDelegate>)managersDelegate {
    if (!manageDelegates) {
        manageDelegates = [[DMWeakReferencesArray alloc] init];
    }
